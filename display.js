...
function buyMaxButton() {
    return upgrades[2] ? `<button class="A" onmousedown="buyMax()">购买最大</button>` : ""
}

function buyMaxAuto() {
    return EU[1] ? `<button class="A" onmousedown="auto = !auto">自动：
${auto ? "是" : "否"}
</button>` : ""
}
...
function finalExamination() {
    return EU[2] ? `<div class="E">你已经进行了
...
function MaxFEButton() {
    return EC[0] ? `<button class="F" onmousedown="MaxFE()">最大期末考试</button>` : ""
}
...
function EC0() {
    return EC[0] ? `<button class="EC" style="background: #c0c000" onmousedown="clickEC(0)">
试卷挑战0：<br>你只有一个自动作业机<br>挑战目标： 3e91 本写好的作业<br>已完成<br>奖励：解锁最大期末考试
</button>` : EU[3] ? `<button class="EC" onmousedown="clickEC(0)">
试卷挑战0：<br>你只有一个自动作业机<br>挑战目标： 3e91 本写好的作业<br>点击这个按钮
${currentChallenge == 0 ? (finishHomeworks.gte(1e91) ? "完成挑战" : "退出挑战") : "进入挑战"}
<br>奖励：解锁最大期末考试
</button>` : ""
}

function EC1() {
    return EC[1] ? `<button class="EC" style="background: #c0c000" onmousedown="clickEC(1)">
试卷挑战1：<br>试卷不会提升你的作业机速度<br>挑战目标： 9e74 本写好的作业<br>已完成<br>奖励：期末考试不重置你的试卷
</button>` : EC[0] ? `<button class="EC" onmousedown="clickEC(1)">
试卷挑战1：<br>试卷不会提升你的作业机速度<br>挑战目标： 9e74 本写好的作业<br>点击这个按钮
${currentChallenge == 1 ? (finishHomeworks.gte(1e123) ? "完成挑战" : "退出挑战") : "进入挑战"}
<br>奖励：期末考试不重置你的试卷
</button>` : ""
}
