...
let display0 = select("D0_")
let display1 = select("D1_")
...
let examinationGain = () => finishHomeworks.root(20).sub(1).mul(EC[3] ? new D(8) : new D(1)).
    mul(sleepGainup()).
    floor()
...
let GE = new D(0)
let GEPrice = () => GE.gte(10) ? (new D(256)).pow(GE.sub(8)) : GE.gte(4) ? (new D(4)).pow(GE.sub(2)) : (new D(2)).pow(GE)
let GEGainup = () => (new D(2)).pow(GE)
...
let sleepGain = () => examinationPapers.add(1).log(4).sqrt().div(4).sub(2).mul(GEGainup()).floor()
...
function buyEU(n) {
    let EUPrice = [20, 40, 1600, 8e9, 1e50][n]
...
function clickEC(n) {
    let goal = [1e77, 9e61, 1e125, 3e52][n]
...
function BuyEngine() {
    if (games.gte(GEPrice())) {
        games = games.sub(GEPrice())
        GE = GE.add(1)
    }
}
