...
let examinationSpeedup = () => currentChallenge == 1 ? 
...
let knowledgeSpeedup = () => knowledge.div(10).add(1).pow(6).pow(currentChallenge == 2 ? 0.5 : 1)
let machines = new D(0)
let maxHomework = new D(0)
let price = () => machines
let speedup = () => upgrades[1] ? teachers.sqrt().pow(currentChallenge == 2 ? 0.4 : 1) : new D(1)
let speed = () => machines.pow(currentChallenge == 2 ? 0.8 : 1).
    mul(speedup()).
...
function MaxFE() {
    ...
    let totalEP = EC[1] ? examinationPapers : finalExaminations.pow(3).add(new D(0))
...
function clickEC(n) {
    let goal = [2e95, 9e78, 1e154, 1e300][n]
