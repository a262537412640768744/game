...
let DPrice = (n) => (new D(2)).
    ...
    mul(100)
let DSpeed = (n) => dreamLevels[n - 1].add(dreams[n]).mul((new D(2)).pow(dreamLevels[n - 1]))
...
let sleepSpeedup = () => (new D(4)).pow(sleepPoint.mul(2.25)).pow(DSpeedup())
let sleepGainup = () => (new D(4)).pow(sleepPoint.mul(0.25)).pow(DSpeedup())
...
function sleepReset() {
    if (SRconfirm && !confirm("你确定要睡觉？这会重置你之前的进度！")) return
...
function update() {
    ...
    display1.innerHTML = [homeworkMachines(), changelog(), examination(), sleepDisplay()][choose]
