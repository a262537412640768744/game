function save() {
    ...
    localStorage.setItem("GRconfirm", JSON.stringify(GRconfirm))
    localStorage.setItem("game", games.toString())
    localStorage.setItem("GM", JSON.stringify(GM))
    localStorage.setItem("SRauto", JSON.stringify(SRauto))
    localStorage.setItem("dreamauto", JSON.stringify(dreamAuto))
    localStorage.setItem("SRmultiplyer", SRmultiplyer.toString())
    setTimeout(save, 0)
}
...
function load() {
    ...
    GRconfirm = get("GRconfirm", true, JSON.parse)
    games = get("game", new D(0), n => new D(n))
    GM = get("GM", [false, false, false, false], JSON.parse)
    SRauto = get("FEauto", false, JSON.parse)
    dreamAuto = get("dreamauto", [false, false, false], JSON.parse)
    SRmultiplyer = get("SRmultiplyer", new D(0.025), n => new D(n))
}
...
function reset() {
    ...
    SRauto = false
    dreamAuto = [false, false, false]
    SRmultiplyer = new D(0.025)
    GE = new D(0)
}